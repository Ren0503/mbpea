...
export const deletePost = async (id: string): Promise<any> => {
    ...
    // Delete the post favorites from the favorite's collection.
    await Favorite.find({ post: post._id }).deleteMany();
    // Delete the post favorites from the user's collection.
    post.favorites.map(async (favoriteId) => {
        await User.find({ favorites: favoriteId }).updateMany({ $pull: { favorites: favoriteId } });
    });
